...
export default function OnboardFooter({ screenIndex, onContinue }: OnboardingProps) {
	return (
		<React.Fragment>
			{/* header steps indicator */}
			<View className={`mt-1`} style={styles.stepIndicatorContainer}>
				{[1, 2, 3].map((_step, index) => (
					<View
						key={index}
						style={[
			...
			<View style={styles.wrapper}>
				...
				<Animated.View entering={FadeIn} exiting={FadeOut} style={styles.footer}>
					<View style={styles.buttonsRow}>
						<Text onPress={() => router.replace("/(Home)")} style={styles.buttonText}>
							Skip
						</Text>
