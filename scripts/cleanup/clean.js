...
${colors.reset}`);
	...
	displayOptions() {
		console.log(`${colors.bright}Select what you want to clean:${colors.reset}\n`);

		this.cleanupOptions.forEach((option) => {
			console.log(`${option.color}${option.key}${colors.reset}. ${option.description}`);
		});
	...
	findDirectories(pattern) {
		try {
			...
			if (buildPatterns.includes(patternName) || cachePatterns.includes(patternName)) {
	...
	async confirmDeletion(paths, optionName) {
		...
		console.log(`${colors.bright}Total size: ${this.formatBytes(totalSize)}${colors.reset}`);
	...
	deletePaths(paths) {
		...
		paths.forEach((dirPath) => {
			try {
				...
				console.log(`${colors.red}Error deleting ${dirPath}: ${error.message}${colors.reset}`);
			}
		});
		return deleted;
	}
	...
	async cleanSelected(selections) {
		...
		for (const selection of selections) {
			...
			if (await this.confirmDeletion(paths, option.name)) {
				...
				console.log(`${colors.green}✓ Deleted ${deleted} ${option.name} items${colors.reset}`);
			} else {
		...
		if (totalDeleted > 0) {
			...
			console.log(`${colors.green}Total items deleted: ${totalDeleted}${colors.reset}`);
			console.log(
